...
export default function AdminPage() {
  ...
  const passwordRef = useRef(""); // holds the verified password for API calls
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/books")
      ...
      .catch(() => {
        setFetchError("Could not connect to admin API. Is the dev server running?");
        setLoading(false);
      });
  }, [authed]);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLoginError("Could not reach admin API. Is the dev server running?");
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch("/api/admin/delete-book", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ slug: deleteTarget.slug, password: passwordRef.current }),
      });
      ...
      <div className="min-h-screen flex items-center justify-center bg-[var(--color-bg)] px-4">
        ...
        >
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-[var(--color-text)]">Admin</h1>
            <p className="text-[var(--color-text-muted)] text-sm mt-1">Skriuwer.com book manager</p>
            <p className="text-[var(--color-text-dim)] text-xs mt-3">
              Only works when running <code className="bg-white/5 px-1 rounded">next dev</code>
            </p>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg)]">
      ...
      {toast && (
        <div
          className={`fixed top-5 right-5 z-50 px-4 py-3 rounded-xl shadow-xl text-sm font-medium transition-all ${
            toast.ok
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-40 px-4">
          ...
          >
            ...
            <p className="text-[var(--color-text)] font-medium mb-0.5 truncate">{deleteTarget.title}</p>
            ...
            <p className="text-[var(--color-text-dim)] text-xs font-mono mb-6">
              ASIN: {deleteTarget.asin || "—"} · slug: {deleteTarget.slug}
            </p>
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={deleting}
                className="flex-1 py-2.5 rounded-xl text-[var(--color-text)] font-semibold text-sm transition-colors"
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-[var(--color-text-muted)] text-sm mt-1">
              {books.length} books ·{" "}
              <span className="text-[var(--color-orange)]">{books.filter((b) => b.isOwnBook).length} own</span>
              {" · "}
          ...
          <div className="flex items-center gap-4">
            {deleted.length > 0 && (
              <span className="text-amber-400 text-sm font-medium">
                ⚠ {deleted.length} deleted — commit &amp; push when done
              </span>
            )}
            <button
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          <input
            ...
            className="flex-1 min-w-56 px-3 py-2.5 rounded-xl text-sm text-[var(--color-text)] outline-none transition-colors"
            style={{
        ...
        {/* Table */}
        {loading ? (
          <div className="py-24 text-center text-[var(--color-text-muted)] text-sm">
            Loading books…
          </div>
        ) : (
          <div
          ...
          >
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: "var(--color-surface-light)", borderBottom: "1px solid var(--color-border)" }}>
              ...
              <tbody>
                {filtered.map((book, i) => (
                  <tr
                    ...
                    style={{
                      background: i % 2 === 0 ? "var(--color-surface)" : "var(--color-surface-light)",
                    ...
                    <td className="px-3 py-2">
                      {book.coverImage ? (
                        ...
                        <img
                          ...
                          onError={(e) => {
                            if (book.coverImageFallback) {
                              (e.currentTarget as HTMLImageElement).src = book.coverImageFallback;
                            }
                          }}
                        />
                      ) : (
                        <div className="w-8 h-12 rounded" style={{ background: "var(--color-surface-light)" }} />
                      )}
                    </td>
          ...
          >
            ...
            <p className="text-xs text-amber-500/60">
              ...
              <code className="bg-white/5 px-1 rounded">
                git add data/books.json &amp;&amp; git commit -m &quot;Remove books&quot; &amp;&amp; git push
              </code>{" "}
              to deploy.
            </p>
          </div>
